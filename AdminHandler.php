...
if (isset($_POST['login'])) {
    ...
    if (count($errors) == 0) {
        ...
        $result = $db->connect->query("SELECT * FROM admin WHERE userName = '$userName' and password = '$pass' LIMIT 1 ");
        $id = $db->connect->query("SELECT id FROM admin WHERE userName = '$userName' LIMIT 1");
